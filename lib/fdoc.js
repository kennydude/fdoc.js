...
function get_template(file){
	return hogan.compile(fs.readFileSync(__dirname + file).toString())
}
...
// Copy Foundation
foundation = fs.readFileSync(__dirname + "/foundation.min.css");
fs.writeFileSync(path.join(out_dir, "foundation.min.css"), foundation);

template = get_template("/fdoc.mustache");
partials = {
	"property.mustache" : get_template("/property.mustache"),
	"type.mustache" : get_template("/type.mustache")
};
